"""Public V3.47 bounded Evidence claim-binding DEV contract."""
...
BENCHMARK_VERSION = "v347-bounded-evidence-claim-binding-dev-v1"
BASELINE_VERSION = "evidence-v345-decision-scope-candidate"
...
CLAIM_CASES = (
    "ATTRIBUTE_ALIAS_BINDING",
    "LED_ROW_OWNERSHIP",
    "FACTORY_DEFAULT_QUALIFIER",
    "SECTION_OWNERSHIP",
    "REFERENCE_OWNERSHIP",
    "BEST_CONFLICT_SELECTION",
)
HARD_NEGATIVE_TYPES = (
...
def validate_dataset(payload: dict) -> tuple[str, ...]:
    ...
    for key in (
        ...
        "uses_v343_documents", "uses_v345_documents",
    ):
    ...
    required = {
        "query_id", "query", "document_id", "expected", "claim_case",
        "target", "relation", "attribute", "value_or_action",
        "relevant_chunk_ids", "confidence", "identity_expected",
        "identity_compatible", "hard_negative_type", "parser_recoverable",
    }
    ...
    for index, row in enumerate(rows):
        ...
        case = str(row.get("claim_case", ""))
        if case not in CLAIM_CASES:
            errors.append(f"CLAIM_CASE:{qid}")
        cases[case] += 1
...
def acceptance(
    baseline: dict[str, Any], candidate: dict[str, Any], *, unsafe_veto: int,
    unsafe_relax: int, baseline_hard_negative_fa: int,
    candidate_hard_negative_fa: int, baseline_replay_mismatches: int,
    runtime_integrity: bool, candidate_frozen: bool, one_shot: bool,
) -> dict[str, Any]:
    ...
    checks = {
        ...
        "identity_hard_negative_fa_not_increased": candidate_hard_negative_fa <= baseline_hard_negative_fa,
