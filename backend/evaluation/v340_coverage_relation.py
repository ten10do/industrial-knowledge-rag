"""Public V3.40 DEV benchmark, coverage-relation taxonomy, metrics, acceptance.
...
BENCHMARK_VERSION = "v340-coverage-relation-dev-v1"
QUERY_COUNT = 80
ANSWER_COUNT = 40
ABSTAIN_COUNT = 40

# §Coverage taxonomy — eight required relation types, ten queries each.
COVERAGE_RELATIONS = (
    "DIRECT_PARAMETER",
    "SAME_SECTION_REFERENCE",
    "CROSS_SECTION_REFERENCE",
    "PRODUCT_FAMILY_INHERITANCE",
    "MODULE_PARENT_RELATION",
    "CONFIGURATION_DEPENDENCY",
    "NEGATIVE_SCOPE_CONFLICT",
    "VALUE_SCOPE_CONFLICT",
)

# Per-relation ANSWER quota (the remainder of each ten-query slice is ABSTAIN).
# The two conflict relations are ABSTAIN-only by definition.
RELATION_ANSWER_QUOTA = {
    "DIRECT_PARAMETER": 6,
    "SAME_SECTION_REFERENCE": 7,
    "CROSS_SECTION_REFERENCE": 7,
    "PRODUCT_FAMILY_INHERITANCE": 7,
    "MODULE_PARENT_RELATION": 7,
    "CONFIGURATION_DEPENDENCY": 6,
    "NEGATIVE_SCOPE_CONFLICT": 0,
    "VALUE_SCOPE_CONFLICT": 0,
}


class CoverageRelation(str, Enum):
    """Modeled evidence-coverage relation between a query and its evidence."""
...
COVERAGE_TO_RELATION = {
    "DIRECT_PARAMETER": CoverageRelation.DIRECT.value,
    "SAME_SECTION_REFERENCE": CoverageRelation.REFERENCED.value,
    "CROSS_SECTION_REFERENCE": CoverageRelation.REFERENCED.value,
    "PRODUCT_FAMILY_INHERITANCE": CoverageRelation.INHERITED.value,
    "MODULE_PARENT_RELATION": CoverageRelation.DEPENDENT.value,
    "CONFIGURATION_DEPENDENCY": CoverageRelation.DEPENDENT.value,
    "NEGATIVE_SCOPE_CONFLICT": CoverageRelation.UNSUPPORTED.value,
    "VALUE_SCOPE_CONFLICT": CoverageRelation.UNSUPPORTED.value,
}

# Reason codes the candidate may emit (validated vocabulary).
REASON_CODES = (
    # safe relaxations
    "DIRECT_PARAMETER_SUPPORTED",
    "SAME_SECTION_SUPPORTED",
    "CROSS_SECTION_LINK_EXPLICIT",
    "FAMILY_INHERITANCE_SUPPORTED",
    "MODULE_PARENT_SUPPORTED",
    "CONFIGURATION_DEPENDENCY_SUPPORTED",
    # refusals
    "DIRECT_PARAMETER_INSUFFICIENT_COVERAGE",
    "SECTION_ANCHOR_MISSING",
    "CROSS_SECTION_LINK_MISSING",
    "FAMILY_INHERITANCE_UNSUPPORTED",
    "MODULE_PARENT_UNSUPPORTED",
    "CONFIGURATION_DEPENDENCY_MISSING",
    "NEGATIVE_SCOPE_CONFLICT",
    "VALUE_SCOPE_CONFLICT",
    # inherited guards (never relaxed)
    "NEGATED_OR_EXCLUSIVE_PROPOSITION",
    "PROCEDURE_RELAXATION_FORBIDDEN",
    "IDENTITY_BOUNDARY_PRESERVED",
    "NON_RELAXABLE_EVIDENCE_REASON",
    "NO_LOCAL_CANDIDATE_RELATION",
    "LEXICAL_COVERAGE_INSUFFICIENT",
)
...
def validate_dataset(payload: dict) -> ValidationReport:
    ...
    for index, row in enumerate(rows):
        ...
        if row.get("expected") == "ANSWER":
            answer_distribution[coverage] += 1
        if row.get("expected") not in {"ANSWER", "ABSTAIN"}:
            errors.append(f"EXPECTED:{index}")
        if coverage in ("NEGATIVE_SCOPE_CONFLICT", "VALUE_SCOPE_CONFLICT") and row.get("expected") != "ABSTAIN":
            errors.append(f"CONFLICT_MUST_ABSTAIN:{index}")
    ...
    for coverage in COVERAGE_RELATIONS:
        if distribution[coverage] != 10:
            errors.append(f"COVERAGE_COUNT:{coverage}:{distribution[coverage]}")
        if answer_distribution[coverage] != RELATION_ANSWER_QUOTA[coverage]:
            errors.append(
                f"RELATION_ANSWER_QUOTA:{coverage}:"
                f"{answer_distribution[coverage]}!={RELATION_ANSWER_QUOTA[coverage]}"
            )
...
def relation_slices(records: list[dict], decision_key: str = "decision") -> dict[str, Any]:
    """Per-coverage-relation metrics plus the modeled relation and relax counts."""
    slices: dict[str, Any] = {}
    for coverage in COVERAGE_RELATIONS:
        subset = [row for row in records if row.get("coverage") == coverage]
        if not subset:
            continue
        entry: dict[str, Any] = {
            "modeled_relation": COVERAGE_TO_RELATION[coverage],
            **metrics(subset, decision_key),
        }
        if decision_key == "decision":
            entry["relaxed_from_baseline"] = sum(
                row.get("baseline_decision") == "ABSTAIN" and row.get("decision") == "ANSWER"
                for row in subset
            )
            entry["unsafe_relax"] = sum(
                row.get("expected") == "ABSTAIN"
                and row.get("baseline_decision") == "ABSTAIN"
                and row.get("decision") == "ANSWER"
                for row in subset
            )
        slices[coverage] = entry
    return slices
